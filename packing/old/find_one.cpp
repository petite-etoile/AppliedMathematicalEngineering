...
void rec(int depth, int& cnt, int now_h, vector<int> const& indices, bitset<grid_size> const& grid, bitset<grid_size> const& mask){
    if(cnt>0) return;
...
int main(){
    ...
    for(int w=6; w>=3; w--){
        ...
        }while(next_permutation(indices.begin(), indices.end()) and cnt==0);
    }
